...
class HiddenNotices {
  ...
  static Future<void> loadHidden() async {
    ...
    _hidden.clear();
    _hidden.addAll(data.map((jsonStr) {
  ...
  static Future<void> add(Notice notice) async {
    if (!_hidden.any((n) => n.title == notice.title && n.startDate == notice.startDate)) {
  ...
  static bool contains(Notice notice) {
    return _hidden.any((n) => n.title == notice.title && n.startDate == notice.startDate);
  }
